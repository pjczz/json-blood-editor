...
let translation = reactive<bloodJSonExplainType>({
  ability: "能力",
  edition: "版本",
  firstNight: "首夜行动顺序",
...
const chineseName = (item: keyof bloodJSonExplainType) => {
...
store.bloodJSon.forEach((item: bloodJSonType, index: number) => {
  if (route.params.id == item.id) {
    Object.keys(item).forEach((ele: keyof bloodJSonType, index:number) => {
...
const submitForm = (formEl: FormInstance | undefined) => {
  if (!formEl) return;
  store.bloodJSon.forEach((item:bloodJSonType, index) => {
    if (route.params.id == item.id) {
